...
func TestMemoryGlossaryContainsCommittedEntity(t *testing.T) {
	...
	commitParams := MemoryCommitParams{
		...
		Result: extract.Result{
			EpisodeSummary: "seed",
			Entities: []extract.Ent{
				{Name: "book-system", Type: "service", Aliases: []string{"book-system pipeline"}},
			},
		},
	}
	if _, err := d.handleMemoryCommit(ctx, mustJSON(t, commitParams)); err != nil {
	...
	want := "book-system: book-system pipeline"
	found := false
	for _, l := range lines {
		if l == want {
			found = true
...
func TestMemoryFactsAndEpisodesResolveAlias(t *testing.T) {
	...
	commitParams := MemoryCommitParams{
		...
		Result: extract.Result{
			EpisodeSummary: "book-system deployed to hermes-mini",
			Entities: []extract.Ent{
				{Name: "book-system", Type: "service", Aliases: []string{"book-system-cli"}},
				{Name: "hermes-mini", Type: "machine"},
			},
			Facts: []extract.Fct{
				{Src: "book-system", Relation: "deployed_on", Dst: "hermes-mini", Fact: "runs there", Confidence: 0.8},
	...
	factsRes, err := d.handleMemoryFacts(ctx, mustJSON(t, MemoryFactsParams{Slug: "book-system-cli"}))
	if err != nil {
		t.Fatalf("handleMemoryFacts(alias %q): %v", "book-system-cli", err)
	}
	...
	episodesRes, err := d.handleMemoryEpisodes(ctx, mustJSON(t, MemoryEpisodesParams{Entity: "book-system-cli"}))
	if err != nil {
		t.Fatalf("handleMemoryEpisodes(alias %q): %v", "book-system-cli", err)
	}
