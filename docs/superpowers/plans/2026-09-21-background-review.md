...
- [x] Validate and install: focused tests, race tests, full tests, vet, independent review, local binary rollback/install, existing launchd restart, real CLI/MCP/provider checks and memory record of outcome.
...
- [ ] Real-model regression detection and clean-control evaluation: first Z.ai attempt returned HTTP 429; report preserved, installed provider block prevents retries until explicit operator resume. No model-quality claim has been made.
